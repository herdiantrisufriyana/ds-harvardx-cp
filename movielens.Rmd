...
```{r Dataset dimension, echo=FALSE}
...
```{r Functions to save or read and download from github for split RDS, include=FALSE}
# Save RDS of table variable
saveRDS_split=function(object,file,split_n,ext='.rds'){
  seq(1,nrow(object),len=split_n+1) %>%
    floor() %>%
    sapply(X=seq(length(.)-1),Y=.,Z=object,K=file,L=ext,FUN=function(X,Y,Z,K,L){
      saveRDS(Z[Y[X]:(Y[X+1]-ifelse(X==(length(Y)-1),0,1)),],paste0(K,X,L))
      paste0(K,X,L)
    })
}
...
download_from_my_github=function(file_list){
  file_list=
    sapply(file_list,function(x){
      download.file(paste0('https://raw.github.com/herdiantrisufriyana/ds-harvardx-cp/master/',x),x)
      x
    })
...
```{r Install specific packages with specific version of BiocManager, include=FALSE}
BiocManager::install(
...
```{r Set theme for plot, include=FALSE}
ds_theme_set()
```
...
# 
# if(!require(tidyverse) & packageVersion('tidyverse')=='1.3.0') BiocManager::install('tidyverse',update=F)
# if(!require(caret) & packageVersion('caret')=='6.0.86') BiocManager::install('caret',update=F)
# if(!require(data.table) & packageVersion('data.table')=='1.13.0') BiocManager::install('data.table',update=F)
# 
# library(tidyverse)
# library(caret)
# library(data.table)
...
# }
# if(!'validation.rds' %in% list.files()) saveRDS(validation,'validation.rds')
```
...
```{r Check missing value, echo=TRUE}
...
```{r Check outcome 0-to-5 completeness, echo=TRUE}
...
```{r Create a function to preprocess, include=FALSE}
...
To develop prediction models, we only used edx as training set; thus, an internal validation index was maded to subset this training set to evaluate predictive performances without using validation set. We did not make a different dataset, but, only use the index if we need an internal validation set.

```{r Split train_set or edx into a train and validation set (10-fold cross validation), echo=TRUE}
...
```{r A function to prevent the same movie or user between the training and internal validation, include=FALSE}
...
```{r User-centered additional features, echo=TRUE}
...
We will only explore edx that is already rename as train_set. By summary table, we can see at glance that number of days and number of movie or the rating is not scaled. We will scale them later.

```{r Training set summary, echo=FALSE}
train_set %>%
  group_by(userId) %>%
...
```{r Rating average as general bias, echo=FALSE, fig.height=3.5, fig.width=5}
train_set %>%
  ggplot(aes(rating)) +
...
```{r Rating average per movie, echo=FALSE, fig.height=7, fig.width=5}
train_set %>%
...
```{r Rating average per user, echo=FALSE, fig.height=7, fig.width=5}
train_set %>%
...
```{r Rating average per user after removing three kind of averages, echo=FALSE, fig.height=7, fig.width=5}
train_set %>%
...
```{r Bivariate distribution between rating and user-centered features globally, echo=FALSE, fig.height=3.5, fig.width=10.5}
# suppresWarnings(set.seed(33)) # if using R 3.5 or earlier
suppressWarnings(set.seed(33,sample.kind = 'Rounding')) # if using R 3.6 or later
...
```{r Bivariate distribution between rating and each user-centered features, echo=FALSE, fig.height=5, fig.width=5}
train_set %>%
  mutate(rating=rating-mean(rating)) %>%
  group_by(movieId) %>%
  mutate(rating=rating-mean(rating)) %>%
...
```{r A movie rating may be predicted by its neighbors, echo=FALSE}
mat=
  train_set %>%
  mutate(rating=rating-mean(rating)) %>%
  group_by(movieId) %>%
...
```{r More number of rating more higher the rating, echo=FALSE}
train_set %>%
  mutate(rating=rating-mean(rating)) %>%
  group_by(movieId) %>%
  mutate(rating=rating-mean(rating)) %>%
...
```{r Visualization of user-movie matrix and the matrices of the factor vectors, echo=FALSE, fig.height=3.5, fig.width=10.5}
mat=
  train_set %>%
  mutate(rating=rating-mean(rating)) %>%
  group_by(movieId) %>%
...
```{r Find out factor vectors that may be associated with rating, echo=FALSE}
train_set %>%
...
```{r Baseline model by average + movie bias + user bias, echo=FALSE}
...
```{r Save regularization data, echo=TRUE}
reg_files=
  seq(10) %>%
...
```{r Get mu with movie and user bias to compute y_hat and residual of rating by whole train set, echo=TRUE}
mu=
  train_set %>%
  summarize(mu=mean(rating))
...
```{r Save baseline model, echo=TRUE}
saveRDS(mu,'mu.rds')
...
```{r User-centered additional features used to predict residuals after baseline model, echo=TRUE}
train_set=
  train_set %>%
  cbind(mu) %>%
  left_join(movie_b,by='movieId') %>%
...
We will also used movie bias as one more additional features; thus this model had four features:
...
```{r Create a function to run stratified ridge regression model with tidy results, echo=TRUE}
...
```{r Conduct ridge regression for each user by parallel computing, echo=TRUE}
# {
...
```{r Save user ridge regression, echo=TRUE}
...
Below we tidy up the ridge regression results, incorporate it into training set, then estimate the RMSE using 10-fold cross validation. We just used the weights and computed manually instead of using 'predict' function. Therefore, just by subsetting the training set using the index, we can used the internally-validated prediction (y_hat) to estimate the RMSE.
...
```{r User-centered boosting by ridge regression using additional features, echo=TRUE}
...
```{r Prepare movie-residual table and movie-movie pairing table for similarity calculation, echo=TRUE}
...
```{r Compute cosine similarity for each movie pair step-by-step to tradeoff computational capacity and time, echo=TRUE}
# cat('Started:',as.character(now()),'\n')
# 
...
```{r Continue to compute cosine similarity (non-expensive steps), echo=TRUE}
...
```{r Prepare neighborhod modeling, echo=TRUE}
...
```{r Find k most similar neighbors of which average minimize RMSE using 10-fold CV by parallel computing, echo=TRUE}
# {
...
```{r Save comparison of movie k-neigbors model, echo=TRUE}
...
```{r Get the best k and show the plot, echo=FALSE}
...
```{r Use the best k to construct training set of movie neighbors sparse linear model, echo=TRUE}
...
```{r Create a function to run k-neighbors lasso regression model with tidy results, echo=TRUE}
...
```{r Conduct lasso regression for each movie  by parallel computing, echo=TRUE}
# {
...
```{r Save movie lasso regression, echo=TRUE}
...
```{r Movie-centered boosting by lasso regression using k-neighbors, echo=TRUE}
...
```{r Get movie k-neighbor residuals for training set  by parallel computing, echo=TRUE}
# {
...
```{r Save k-neighbor residuals for training set, echo=TRUE}
...
```{r Continue to evaluate movie-centered boosting by lasso regression using k-neighbors, echo=TRUE}
...
```{r Function to train implicit neighbor rating model, echo=TRUE}
...
```{r Fit implicit neighbor rating of each user using linear regression by parallel computing, echo=TRUE}
# {
...
```{r Save user implicit linear regression, echo=TRUE}
...
```{r User implicit neighbor rating model, echo=TRUE}
...
```{r Prepare matrices and a function for standard SVD model, echo=TRUE}
...
```{r Determine number of latent factor for standard SVD for each fold by parallel computing, echo=TRUE}
...
```{r Save standard SVD latent factor data, echo=TRUE}
...
```{r Standard SVD latent factor that minimizes RMSE, echo=FALSE}
fac_ssvd_dt=
  fac_ssvd %>%
...
```{r Standard SVD model, echo=TRUE}
...
```{r Compile previous models as a list}
...
```{r Ensemble model by majority vote}
ensemble$majority=
  matrix(c(
...
```{r Ensemble model by average rating}
ensemble$average=
  matrix(c(
    ensemble$baseline$y_hat,
    ensemble$user_ridge$y_hat,
...
```{r Ensemble model by stacking (data preparation)}
ensemble$stacking=
  data.table(
    id=ensemble$baseline$id,
    movieId=ensemble$baseline$movieId,
...
Below you can see the training function. We did not apply parallelism in this part beause we cannot determine the loops. Fortunately, the computation is fairly affordable with single core processing unit.

```{r Create a function to run elastic net regression model for stacking ensemble with tidy results}
...
```{r Save ensemble staking elastic net regression model}
...
```{r Ensemble stacking elastic net regression model (tidy up and incorporating to training set)}
...
```{r Preprocessing validation dataset and predict the rating using only the best model, echo=TRUE}
...
Look at test_set that was constructed by the code above and that being presented by the code below. The best model can achieve the goal. The RMSE was 0.85706. This is lower than the target of <0.86490 by 0.00784. So, we did it, we have achieved the goal :))

```{r Validation RMSE of test set, echo=TRUE}
cat('RMSE = ',round(RMSE(test_set$y_hat,test_set$rating),5),'\n',
    '25 points: RMSE <0.86490 :))',sep='')
```

Just for sanity check. Let's use the original 'validation' set instead of processing it into test_set. But, stick to use prediction from test_set.

```{r Validation RMSE of original validation set, echo=TRUE}
...
```{r The best model as a prediction function, echo=FALSE}
cat('y_hat','\n',
...
```{r Weight distribution per feature in user ridge regression model, echo=FALSE}
train_set %>%
...
```{r Distribution of rating per RMSE group and set, echo=FALSE}
rbind(
    train_set %>%
      mutate(set='train') %>%
...
```{r Distribution of residual per rating in training set, echo=FALSE, fig.height=3.5, fig.width=10.5}
train_set %>%
  ggplot(aes(residual)) +
